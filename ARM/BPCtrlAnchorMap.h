...
public:
  ...
  void Initialize(HWND hWndParent, DWORD dwFlags/*=0*/) {
    ...
    if (dwFlags & ANIF_SIZEGRIP) {
      ...
      m_hWndSizeGrip  = ::CreateWindow("ScrollBar", "", WS_CHILD | WS_VISIBLE | SBS_SIZEGRIP, m_rcClient.right-sz1.cx, m_rcClient.bottom-sz1.cy, sz1.cx, sz1.cy, m_hWndParent, NULL, NULL, 0);
    ...
    if (dwFlags & ANIF_CALCSIZE) {
      ...
      for (iCtrl=0; iCtrl < m_nCtrlCount; iCtrl++) {
        ...
        if (m_Ctrl[iCtrl].rect.right > rcMaxBR.right) rcMaxBR.right = (LONG)m_Ctrl[iCtrl].rect.right;
        if (m_Ctrl[iCtrl].rect.bottom > rcMaxBR.bottom) rcMaxBR.bottom = (LONG)m_Ctrl[iCtrl].rect.bottom;
