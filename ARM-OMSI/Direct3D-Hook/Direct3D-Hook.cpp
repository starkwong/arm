...
*/
typedef IUnknown* (WINAPI *MyDirect3DCreate9_t)(UINT SDKVersion);
...
*/
IUnknown* WINAPI MyDirect3DCreate9(UINT SDKVersion);
...
SDLLHook D3DHook = 
{
	...
	{
		{ "D3D9.DLL","Direct3DCreate9", MyDirect3DCreate9},
...
HRESULT WINAPI MyDirectInputCreateA(HINSTANCE hinst, DWORD dwVersion, LPDIRECTINPUTA *ppDI, LPUNKNOWN punkOuter) {
	...
	if (ovi.dwMajorVersion==6 && ovi.dwMinorVersion>=1) {*/
		...
		*ppDI=new MyDirectInput(*ppDI);
		hWndMsg=FindWindow("ARMOMSIDISPMSGCLASS","ARMOMSIDISPMSGWND");
		/*
	} else
...
IUnknown* WINAPI MyDirect3DCreate9(UINT SDKVersion) {
	/*
	LPDIRECT3D9 lpD3D9=Direct3DCreate9(SDKVersion);
	if (lpD3D9) {
		lpD3D9=new MyDirect3D9(lpD3D9);
