...
bool RedirectIAT( SDLLHook* DLLHook, PIMAGE_IMPORT_DESCRIPTOR pImportDesc, PVOID pBaseLoadAddr )
{
	...
	if ( pImportDesc->OriginalFirstThunk == 0 ) {
		OutputDebugString("No Import Names Table!");
        return false;
	}
    ...
    {
		OutputDebugString("Protection Removal Failed!\n");
        return false;
    }
    ...
    if ( DLLHook->UseDefault )
    {
		...
		if ( !pStubs ) {
			OutputDebugString("Redirection stub allocation failed!\n");
            return false;
		}
    }
    ...
    while ( pIteratingIAT->u1.Function )
    {
        ...
        if ( !IMAGE_SNAP_BY_ORDINAL( pINT->u1.Ordinal ) )  // import by name
        {
            ...
            while ( FHook->Name )
            {
				//OutputDebugString( (char*)FHook->Name);
...
bool HookAPICalls( SDLLHook* Hook )
{
    ...
    while ( pImportDesc->FirstThunk )
    {
		...
		for (c=0; Hook->Functions[c].dllName; c++) {
			if ( lstrcmpi( pszImportModuleName, /*Hook->Name*/Hook->Functions[c].dllName ) == 0 )
			{
				OutputDebugString( "Found " );
				OutputDebugString( /*Hook->Name */ Hook->Functions[c].dllName);
				OutputDebugString( "...\n" );
