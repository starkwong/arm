...
extern void InstallHook(LPPROCESS_INFORMATION pi);
...
PLUGININFO pluginInfo = {"Background Display",MAKELPARAM(MAKEWORD(1,3),MAKEWORD(0,0)),MAKELPARAM(MAKEWORD(0,0),MAKEWORD(0,0)),__DATE__" "__TIME__};
...
#define CLASSNAME "ARMBGDISPMSGCLASS"
#define WNDNAME "ARMBGDISPMSGWND"

BOOL PluginLoad(PLUGINLINK* plink) {
	OutputDebugString("Initializing Background Display...\n");
	pluginLink=plink;

	CreateServiceFunction("BGDisplay\\ResetTimer",ResetTimer);
	OutputDebugString("Background Display Loaded\n");
	return TRUE;
}
...
BOOL PluginUnload() {
	...
	*/
	...
	//OutputDebugString(SetThreadPriority(GetCurrentThread(),THREAD_PRIORITY_NORMAL)?"Priority set to Normal":"Unable to set priority!");
	VSFilterFix(false);
	OutputDebugString("Background Display Module Unloaded\n");
	return TRUE;
}
	...
	if (RegOpenKeyEx(HKEY_CLASSES_ROOT,"CLSID\\{083863F1-70DE-11D0-BD40-00A0C911CE86}\\Instance\\{9852A670-F845-491B-9BE6-EBD841B8A613}",0,KEY_SET_VALUE|KEY_QUERY_VALUE,&hKey)==ERROR_SUCCESS) {
		...
		if (fix) {
			...
			if (RegQueryValueEx(hKey,"CLSID",NULL,NULL,(LPBYTE)szTemp+1,&dwData)==ERROR_SUCCESS) {
				if (szTemp[1]=='{') {
					OutputDebugString("ARMBGDisplay: VSFilterFix() disabling VSFilter\n");
					*szTemp=':';
			...
			if (RegQueryValueEx(hKey,"KumaARMSaved",NULL,NULL,NULL,NULL)==ERROR_SUCCESS) {
				OutputDebugString("ARMBGDisplay: VSFilterFix() enabling VSFilter\n");
...
int ExecuteEvent(WPARAM wParam, LPARAM lParam) {
	if (wParam==0)
		InstallHook(/*(LPPROCESS_INFORMATION)lParam*/);
	else
		RemoveHook();
...
BOOL ModulesLoaded() {
	...
	hEvent=CreateEvent(NULL,FALSE,TRUE,"BSASBGDisplayEvent");
